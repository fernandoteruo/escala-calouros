...
function peopleAdjustCalc_(people, depts) {
	...
	for (i = 0; i < people.length; i++) {
		...
		if (people[i].shift == "SD") {
			for (j = 0; j < depts.length; j++) {
				if (people[i].sexo == "H" && !checkAlready_(people[i], depts[j]) && depts[j].presentMen < depts[j].minMen && depts[j].presentAll + 1 <= depts[j].maxAll && depts[j].presentAll != moreAll && depts[j].presentWomen != moreWomen) {
					people[i].shift = depts[j].name;
					break;
				} else if (people[i].sexo == "M" && !checkAlready_(people[i], depts[j]) && depts[j].presentWomen < depts[j].minWomen && depts[j].presentAll + 1 <= depts[j].maxAll && depts[j].presentAll != moreAll && depts[j].presentMen != moreMen) {
					people[i].shift = depts[j].name;
					break;
	...
	for (i = 0; i < people.length; i++) {
		...
		if (people[i].shift == "SD") {
			for (j = 0; j < depts.length; j++) {
				if (people[i].sexo == "H" && !checkAlready_(people[i], depts[j]) && depts[j].presentMen == lessMen && depts[j].presentAll != moreAll && depts[j].presentWomen != moreWomen && depts[j].presentAll + 1 <= depts[j].maxAll) {
					people[i].shift = depts[j].name;
					break;
				} else if (people[i].sexo == "M" && !checkAlready_(people[i], depts[j]) && depts[j].presentWomen == lessWomen && depts[j].presentAll != moreAll && depts[j].presentMen != moreMen && depts[j].presentAll + 1 <= depts[j].maxAll) {
					people[i].shift = depts[j].name;
					break;
	...
	// verifica people que estão em depto que tem falta de um sexo mas ta no maximo geral e troca
	for (i = 0; i < people.length; i++) {
		for (j = 0; j < depts.length; j++) {
			if (depts[j].presentMen < depts[j].minMen && depts[j].presentAll == depts[j].maxAll && people[i].sexo == "M" && !checkAlready_(people[i], depts[j])) {
				depts[j].presentAll--;
				depts[j].presentWomen--;
				people[i] = "SD";
			} else if (depts[j].presentWomen < depts[j].minWomen && depts[j].presentAll == depts[j].maxAll && people[i].sexo == "H" && !checkAlready_(people[i], depts[j])) {
				depts[j].presentAll--;
				depts[j].presentMen--;
				people[i] = "SD";
			}
		...
		if (people[i].turno == "SD") {
			loopDeptsAvailableMin: for (j = 0; j < deptsAvailable.length; j++) {
				...
				if (!checkAlready_(people[i], deptsAvailable[j])) {
					if (people[i].sexo == "H" && deptsAvailable[j].presentMen < deptsAvailable[j].minHomem || people[i].sexo == "M" && deptsAvailable[j].presentWomen < deptsAvailable[j].minMulher || deptsAvailable[j].presentAll < deptsAvailable[j].minGeral) {
						people[i].turno = deptsAvailable[j].name;
	...
	for (i = people.length - 1; i >= 0; i--) {
		if (people[i].turno == "SD") {
			...
			for (j = 0; j < deptsAvailable.length; j++) {
				...
				if (!checkAlready_(people[i], deptsAvailable[j])) {
					if (people[i].sexo == "H" && deptsAvailable[j].presentMen == searchLessMore_(depts).lessMen || people[i].sexo == "M" && deptsAvailable[j].presentWomen == searchLessMore_(depts).lessWomen || deptsAvailable[j].presentAll == searchLessMore_(depts).lessAll) {
						people[i].turno = deptsAvailable[j].name;
						deptsAvailable.splice(j, 1);
